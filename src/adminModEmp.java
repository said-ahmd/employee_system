...
public class adminModEmp extends adminSpecEmpToMod implements ActionListener {
	...
	JTextField fdepart = new JTextField();
	...
	adminModEmp() {
		...
		depart.setBounds(90, 240, 100, 30);
		age.setBounds(90, 290, 100, 30);
		phone.setBounds(90, 340, 100, 30);
		address.setBounds(90, 390, 100, 30);
		

		name.setForeground(Color.decode("#ffffff"));
		id.setForeground(Color.decode("#ffffff"));
		pass.setForeground(Color.decode("#ffffff"));
		depart.setForeground(Color.decode("#ffffff"));
		...
		fdepart.setBounds(180, 240, 200, 30);
		fage.setBounds(180, 290, 200, 30);
		fphone.setBounds(180, 340, 200, 30);
		faddress.setBounds(180, 390, 200, 30);
    ...
    public void actionPerformed(ActionEvent a) {
    	...
    	String depart = fdepart.getText().toString();
    	String age = fage.getText().toString();
    	String address = faddress.getText().toString();
    	String phone = fphone.getText().toString();
    	
        if(a.getSource()==add){
        	if(name.length()==0&&id.length()==0&&depart.length()==0&&age.length()==0&&pass.length()==0) {
        	 JOptionPane.showMessageDialog(f, "enter any data please \"at least one field\" ");
				...
				if(depart.length()!=0) {
					System.out.println(1);
					String query= (" update employee set depart= '"+depart+"' where name= '"+name+"' " );
					st.executeUpdate(query);
				}
				if(phone.length()!=0) {
					System.out.println(1);
				...
				faddress.setText("");fphone.setText("");fage.setText("");fname.setText("");fid.setText("");fdepart.setText("");fpass.setText("");
				st.close();
				con.close();
        		}
				catch (Exception e1) {
